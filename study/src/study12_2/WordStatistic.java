...
public class WordStatistic {
    ...
    File file = new File("H:\\Users\\1\\IdeaProjects\\ssp_basic01\\study\\english.txt");
    Scanner sc = null;
    String regex;
    WordStatistic() {
...
//regex 是由空格、数字和符号(!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~)组成的正则表达式
        regex= "[\\s\\d\\p{Punct}]+";
        try{ sc = new Scanner(file); //创建指向 file 的 sc
            sc.useDelimiter(regex);//sc 调用 useDelimiter(String regex)方法,向参数传递 regex
        }
